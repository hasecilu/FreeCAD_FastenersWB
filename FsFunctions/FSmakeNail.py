...
def makeNail(self, fa):
    ...
    - DIN1144:      Nails for the installation of wood wool composite panels
    ...
    if fa.baseType[:7] == "DIN1143":
        d1, d2, l = fa.dimTable
        nail = self.RevolveZ(makeCountersunkHead(d1, l, 1.1 * d2, csnk_ang=140))
        # Cutter ring
        fm = FSFaceMaker()
        fm.AddPoint(d2 / 2, 0)
        ...
        d, l = fa.dimTable
        nail = self.RevolveZ(makePlainHead(d, l, 20, 1))
        fm = FSFaceMaker()
        ...
        if fa.baseType == "DIN1151-A":
            face = makePlainHead(d, l, 2 * d, d / 4)
        else:
            face = makeCountersunkHead(d, l, 3 * d, csnk_ang=120)
        return self.RevolveZ(face)
    elif fa.baseType[:7] == "DIN1152":
        d, l = fa.dimTable
        return self.RevolveZ(makePlainHead(d, l, 1.25 * d, 1.5 * d))
    elif fa.baseType[:7] == "DIN1160":
        d, l, _, d2 = fa.dimTable
        return self.RevolveZ(makePlainHead(d, l, d2, d / 5))

    raise NotImplementedError(f"Unknown fastener type: {fa.baseType}")


def makePlainHead(dia, length, head_w, head_th, p_angle=20):
...
def makeCountersunkHead(dia, length, head_w, p_angle=20, csnk_ang=120):
    """Make the face for a countersunk head and point end nail."""
    # Calc distance in z to comply with countersunk angle given
